<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px","boxShadow":"0 1px 10px 1px rgba(115,108,203,.1)","margin":"0px","overflow":"hidden","borderRadius":"3px","background":"#fff","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"10px","fontSize":"inherit","textAlign":"left","background":"#fff","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="租房状态" prop="zufangzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">租房状态</label>
						<el-select clearable v-model="searchForm.zufangzhuangtai" placeholder="请选择租房状态" >
							<el-option v-for="(item,index) in zufangzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('zufangjilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('zufangjilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('zufangjilu','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('zufangjilu','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('zufangjilu','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zufangjilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('zufangjilu','退房评价')" @click="tuifangjiluCrossAddOrUpdateHandler(scope.row,'cross','','','zufangzhuangtai','您已退房！','已退房,未退房'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								退房评价
							</el-button>
							<el-button class="btn8" v-if="isAuth('zufangjilu','添加合同')" @click="zulinhetongCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','请勿重复添加！')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								添加合同
							</el-button>
							<el-button class="edit" v-if=" isAuth('zufangjilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zufangjilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<tuifangjilu-cross-add-or-update v-if="tuifangjiluCrossAddOrUpdateFlag" :parent="this" ref="tuifangjiluCrossaddOrUpdate"></tuifangjilu-cross-add-or-update>
...
import tuifangjiluCrossAddOrUpdate from "../tuifangjilu/add-or-update";
import zulinhetongCrossAddOrUpdate from "../zulinhetong/add-or-update";
	export default {
		data() {
			return {
				...
				tuifangjiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			tuifangjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.tuifangjiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zufangjilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tuifangjiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tuifangjiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			payHandler(row){
				this.$storage.set('paytable','zufangjilu');
    ...
    init () {
		this.isPayOptions = "已支付,未支付".split(',')
          this.zufangzhuangtaiOptions = "已退房,未退房".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zufangzhuangtai!='' && this.searchForm.zufangzhuangtai!=undefined){
            params['zufangzhuangtai'] = this.searchForm.zufangzhuangtai
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          zufangyajin: row.zufangyajin,
          dingdanjine: row.dingdanjine,
          zufangzhuangtai: row.zufangzhuangtai,
